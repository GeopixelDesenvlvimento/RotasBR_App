...
function preparations( )
    ...
    -- determine if natural guidance will be used for this prompt
...
function filter_natural_guidance( maneuver )
    ...
    then
        if ( maneuver.ng_type == "TRAFFIC LIGHT" and voice_settings["nguidance_trafficlights"] and _G.trigger ~= "COMMAND" ) then
            show_nguidance = true
        elseif ( maneuver.ng_type == "STOP SIGN" and voice_settings["nguidance_stop_sign"] and _G.trigger ~= "COMMAND" ) then
            show_nguidance = true
...
function provide_natual_guidance(maneuvers, phoneme_string_builder)
    ...
    if _G.trigger == "COMMAND" and ngs[2] ~= nil and ngs[2] ~= ""
    then
        ngs[1] = ""
    end
